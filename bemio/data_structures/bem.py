...
class HydrodynamicData(object):
  ...
  def __init__(self):
    ...
    # strings
    # Flag to determine if the hydrodynamic coefficients dimensional
  ...
  def calc_irf_excitation(self, t_end=100.0, n_t=1001, n_w=1001):
    ...
    self.ex.irf.f = np.zeros(
        [self.ex.mag.shape[0], self.ex.mag.shape[1], self.ex.irf.t.size])
    ...
    pbar_maxval = self.ex.irf.t.size * \
        self.ex.mag.shape[0] * self.ex.mag.shape[1]
    pbar = ProgressBar(widgets=['Calculating the excitation force impulse response function for ' +
                                self.name + ':', Percentage(), Bar()], maxval=pbar_maxval).start()
    count = 1
    for t_ind, t in enumerate(self.ex.irf.t):
      ...
      for i in xrange(self.ex.mag.shape[0]):
        ...
        for j in xrange(self.ex.mag.shape[1]):
          tmp = ex_re_interp[
              i, j, :] * np.cos(self.ex.irf.w * t) - ex_im_interp[i, j, :] * np.sin(self.ex.irf.w * t)
          tmp *= 1. / (2. * np.pi)
  ...
  def calc_irf_radiation(self, t_end=100, n_t=1001, n_w=1001):
    ...
    # Calculate the IRF
    pbar = ProgressBar(widgets=['Calculating the radiation damping impulse response function for ' + self.name +
                                ':', Percentage(), Bar()], maxval=np.size(self.rd.irf.t) * self.rd.all.shape[0] * self.rd.all.shape[1]).start()
    count = 1
  ...
  def calc_ss_radiation(self, max_order=10, r2_thresh=0.95):
    ...
    self.rd.ss.irk_bss = np.zeros(
        [self.am.inf.shape[0], self.am.inf.shape[0], numFreq])
    ...
    pbar = ProgressBar(widgets=['Calculating radiation damping state space coefficients for ' +
                                self.name + ':', Percentage(), Bar()], maxval=self.am.inf.shape[0] * self.am.inf.shape[1]).start()
    count = 0
