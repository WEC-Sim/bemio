...
#
wamit_data_f2 = read(out_file='wamit_data/coer_comp_f2.out')

# Calculate IRF and plot using the wamit module
for i in xrange(wamit_data_f2.body[0].num_bodies): #wamit_data_f2.body[0].num_bodies
 	wamit_data_f2.body[i].calc_irf_radiation(t_end=20.0, n_t=2001, n_w=4001)
	# wamit_data_f2.body[i].calc_ss_radiation(max_order=7, r2_thresh=0.95)
	wamit_data_f2.body[i].calc_irf_excitation(t_end=20.0, n_t=2001, n_w=4001)

# Save the data in the hdf5 format.
write_hdf5(wamit_data_f2,out_file='coer_comp_f2.h5')
